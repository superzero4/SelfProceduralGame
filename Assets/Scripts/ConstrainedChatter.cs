...
public class ConstrainedChatter : ChatInteractor
{
    ...
    {
        RequestArts(_last.data.buildings.Length > 0 ? _last.data.buildings[0].name : _last.data.units[0].name);
    }
    public void RequestArts(string nextName)
    {
        currentChatStep++;
        SubmitToChat("Can you provide me an unique ASCII art for " + nextName, $"I want one and only one detailed ASCII art with at least {_asciiArtResolution}x{_asciiArtResolution} characters.");
    }
}